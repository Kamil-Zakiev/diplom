...
namespace EC_Console
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // генерирование
            // GenerateSemiPrimeNumbers();

            // LogAverageMinTimes();

            // факторизация
            // LogFactoriztionResults();
            ...
            LogEllepticCurvesClassesInfo();

            #endregion
        }

        private static void LogEllepticCurvesClassesInfo()
        {
        ...
        private static void LogFactoriztionResults()
        {
            foreach (var path in Directory.GetFiles("TwoPrimesMultiple"))
            {
                var results = (MultithreadLenstra.UseThreadsParallelism(path, threadsCount: 100));
        ...
        private static void LogAverageMinTimes()
        {
            foreach (var path in Directory.GetFiles("TwoPrimesMultiple"))
            {
                var results = MultithreadLenstra.GetMinTimes(path, threadsCount: 100);
                var group = results
        ...
        private static void GenerateSemiPrimeNumbers()
        {
